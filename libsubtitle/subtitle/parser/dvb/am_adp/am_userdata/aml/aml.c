	...
	pts, int pts_valid, int64_t duration)
{
	...
	pts_in_buffer = (int64_t* )userdata_with_pts;
	*pts_in_buffer = pts;
	...
	pts, int pts_valid, int64_t duration)
{
	...
	if (cc->cap < len) {
		cc->buf = (uint8_t*)realloc(cc->buf, len);
		cc->cap = len;
	}
...
static int aml_process_scte_userdata(AM_USERDATA_Device_t *dev, uint8_t *data, int len, struct userdata_meta_info_t* meta_info)
{
	...
	int head_posi = 0;
	int  field, line, cc1, cc2, size, ptype, ref;
	int bits = 0, array_position = 0;
	uint8_t *p;
	//int left = len;
	int left_bits;
	uint32_t v;
...
static AM_ErrorCode_t aml_open(AM_USERDATA_Device_t *dev, const AM_USERDATA_OpenPara_t *para)
{
	AM_UDDrvData *ud = (AM_UDDrvData*)malloc(sizeof(AM_UDDrvData));
	int r;
